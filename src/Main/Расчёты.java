...
//        }
        ...
        double theta = getTheta(Adding.coordinataX.get(index)-EarthMoving.width/2, Adding.coordinataY.get(index)-EarthMoving.height/2);
        double F = Force(index, r);
//        double Fy = ForceY(index, r);
        double Ax = F*Math.sin(theta + Math.PI)/MassSun;
        double Ay = F*Math.cos(theta + Math.PI)/MassSun;
